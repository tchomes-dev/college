...
82:a := smask;                                  { 1111 1111 00 = MULT } {smask = 0000 0000 1111 1111}
83:a := rshift(a);                              {a = 0000 0000 0111 1111}
84:a := rshift(a);                              {a = 0000 0000 0011 1111}
85:a := band(ir, a);                            {a = 0000 0000 00xx xxxx}
86:mar := sp; rd;                               {mar = sp; read-read}
87:rd; ac:= 0;                                  {mbr = sp[0] ac = 0}
88:b := mbr; if n then goto 96;                 {is b pos or neg; 89/ or \96}
89:a := a+(-1); if z then goto 92;              {b is pos, result must be pos} 
90:alu := ac; if n then goto 95;                {ac += b}
91:ac := ac + b; goto 89;                       {is result pos or neg; 92/ or \95}
92:mbr := ac; wr;                               {result is pos; push to stack}
93:wr; ac := 0;                                 {result on stack, ac = 0}
94:goto 0;                                      {exit}
95:ac :=-1; goto 0;                             {result is neg overflow, no push, load -1 and exit}
96:a := a+(-1); if z then goto 99;              {b is neg, result must be neg}
97:alu := ac; if n then goto 102;               {ac += b}
98:ac := ac + b; goto 96;                       {is result pos or neg}
99:mbr := ac; wr;                               {result is neg, push to stack}
100:wr; ac := 0;                                {result on stack, ac = 0}
101:goto 0;                                     {exit}
102:ac:= -1; goto 0;                            {result is pos overflow, no push, load -1 and exit}
103:a := lshift(1);				{ 1111 1111 01 = RSHIFT }
104:a := lshift(a + 1);
105:a := lshift(a + 1);
106:a := a + 1;
107:b := band(ir, a);
108:b := b + (-1); if n then goto 96;
109:ac := rshift(ac); goto 94;
110:goto 0;
