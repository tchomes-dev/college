...
public class LargeInteger {
  ...
  public LargeInteger add(LargeInteger that) {
    // TODO
	  ...
	  LLNode<Integer> tempThatHead = that.head;
	  LLNode<Integer> temp = null;
	  LLNode<Integer> newList = null;
	  LLNode<Integer> previous = null;
	  int sum;
	  int carry = 0;
	  while (tempHead != null || tempThatHead != null) {
		  sum = carry + (tempHead != null ? tempHead.data : 0)
				  + (tempThatHead != null ? tempThatHead.data : 0);
		  carry = (sum >= 10) ? 1 : 0;
		  sum = sum % 10;
		  ...
		  if (tempThatHead != null) {
			  tempThatHead = tempThatHead.link;
		  }
	  }
	  if (carry > 0) {
		  temp.link = new LLNode<>(carry, null);
	  }
	  
    return new LargeInteger(toString(newList));
  }
  ...
  public LargeInteger multiply(int x) {
    // TODO
	  ...
	  LargeInteger newList = new LargeInteger(toString());
	  if (x != 1) {
		  for (int i = 1; i < x; i++) {
			  newList = add(newList);
		  }
