...
public class BinarySearchTree<T extends Comparable<T>> implements BstInterface<T> {
  ...
  public void balance() {
    ...
    inorderTraverseArray(root, 0, array);
    root = sortedArray2BST(0, array.length - 1, array);
  }

  private void inorderTraverseArray(BstNode<T> node, int index, T[] array) {
    if (node != null) {
      inorderTraverseArray(node.getLeft(), index, array);
      array[index++] = node.getData();
      inorderTraverseArray(node.getRight(), index, array);
    }
  }
